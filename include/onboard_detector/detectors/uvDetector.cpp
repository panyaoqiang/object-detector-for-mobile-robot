...
namespace onboardDetector{
    ...
    UVdetector::UVdetector()
    {
        ...
        this->min_height = 0.0; // default no min height
        this->max_height = 100.0; // default no max height
        this->threshold_point = 3;

        this->threshold_line = 1;
        this->min_length_line = 1;
    ...
    void UVdetector::extract_U_map()
    {
        ...
        for(int col = 0; col < depth_rescale.cols; col++)
        {
            for(int row = 0; row < depth_rescale.rows; row++)
            {
                ...
                // check if the pixel is within any of the ROIs (if ROIs are provided)
                bool in_roi = false;
                if (!this->rois_.empty()) {
                    int orig_x = col / this->col_scale;
                    int orig_y = row;
                    for (const auto& roi : this->rois_) {
                        if (orig_x >= roi.x && orig_y >= roi.y && orig_x < roi.x + roi.width && orig_y < roi.y + roi.height) {
                            in_roi = true;
                            break;
                        }
                    }
                ...
                // If YOLO detects NOTHING, we shouldn't process the whole image.
                // Thus if in_roi is false (either because rois_ is empty or the point is outside), skip it.
                if (!in_roi) continue;
                ...
                {
                    float y_world = (row - this->py) * (depth_rescale_val / 1000.0) / this->fy;
                    if (y_world < this->min_height || y_world > this->max_height) {
                        continue;
                    }
    ...
    void UVdetector::extract_bb()
    {
        ...
        int u_min = this->threshold_point * this->row_downsample;//3*4=12
        // int u_min = 4;//3*4=12 //
        ...
        for(int row = 0; row < this->U_map.rows; row++)
        {
            for(int col = 0; col < this->U_map.cols; col++)
            {
                ...
                if(this->U_map.at<uchar>(row,col) < u_min || col == this->U_map.cols - 1)
                {
                    ...
                    // 线长大于一个像素，且像素总和大于最大像素值的阈值倍数(这里很奇怪呀)，是说总和大于这行像素里面最大值的阈值倍数
        ...
        for(size_t b = 0; b < UVboxes.size(); b++)
        {
            //如果是最顶层的父节点，才开始合并，这里就避免了非最顶层的父节点重复合并
            if(UVboxes[b].id == UVboxes[b].toppest_parent_id)
            {
                for(size_t s = b + 1; s < UVboxes.size(); s++)
                {
                    ...
                    {
                        UVboxes[b] = merge_two_UVbox(UVboxes[b], UVboxes[s]);//各个方向的顶点

                    }
                }
                // check box's size，面积筛选
    ...
    {
        // extract U map from depth + 高斯模糊
        this->extract_U_map();

        // extract bounding box from U map，逐行搜索大于阈值的线，然后合并，最后把合并后的框计算出来
        this->extract_bb();

        // extract bounding box
        this->extract_bird_view();//把上面umap的框转换到真实世界的坐标系的值，单位是cm(这里表示存疑)
    ...
    // U_map丢失了物体在原图中真实的高度信息
    // 拿着在 U-map 里找到的那些含有深度左右边界的候选框，
    // 回到原始的、包含完整图像高度的深度图（或者微缩版深度图 depth_resize）里，
    // 去把物体的上下边界找出来，最终算出完整的、带有高度(Y 轴)信息的 3D 物理包围框，
    // 用单位 “米 (m)” 存储起来。
    ...
    {   
        ...
        for (size_t b = 0; b < this->bounding_box_U.size(); b++) {
            ...
            // 既然知道了这个物体横跨了哪些列，也知道了它离我们有多远。
            // 程序干脆就把那几列原始深度图像素从上往下扫一遍，
            // 看看这一列中，有哪些像素点落在 depth_in_near ~ depth_in_far 这个距离段里。
            // 落在里面的像素，自然就是这个障碍物的“身体”！
            // 外层循环：遍历这个障碍物占据的每一列
            for (int i = x ; i < x + width; i++) { // for several middle coloums
                for (int j = 0; j < depth_resize.rows - 1; j++) { // for each row
                    ...
                    //高度限制--pyq
                    float y_world = (j - this->py) * (depth_resize_val / 1000.0) / this->fy;
                    if (y_world < this->min_height || y_world > this->max_height) {
                        continue;
                    }

                    // 如果这个深度的确落在我们的目标物距离段里！
                    if (float(depth_resize_val) >= depth_in_near && depth_resize_val <= depth_in_far) {
                        // num_check==15，算底下接连几个像素是否在这个物体的深度范围内了
                        for (int check = 0; check < num_check; check++) { // check some more points in the coloum
                            if (j + check + 1 >= depth_resize.rows) break; // 增加防越界保护
                            depth_resize_val = (float(depth_resize.at<unsigned short>(j + check + 1,i))/this->depthScale_)*1000.0;
                            //高度限制--pyq
                            float check_y_world = (j + check + 1 - this->py) * (depth_resize_val / 1000.0) / this->fy;
                            if (check_y_world < this->min_height || check_y_world > this->max_height) {
                                break;
                            }
                            ...
                            // 如果连续 num_check 个像素都在深度段内，说明找到了坚实的物体表面，拓展 y_up 和 y_down
    ...
    void UVdetector::display_U_map()
    {
        ...
        if(this->show_bounding_box_U)
        {
            ...
            cv::minMaxIdx(this->U_map_show, &temp_min, &temp_max);
            ...
            if (std::chrono::duration_cast<std::chrono::seconds>(now - last_print_time).count() >= 1) {
                ...
                fflush(stdout); // Added fflush to fix no output issue in ROS
                last_print_time = now;
            }
            double min = 0.0;
            double max = 255.0; // 固定min、max值，方便调试。你可以修改这个255.0为你预期的最大值
            // cv::minMaxIdx(this->U_map_show, &min, &max);
            cvtColor(this->U_map_show, this->U_map_show, cv::COLOR_GRAY2RGB);
            cv::convertScaleAbs(this->U_map_show, this->U_map_show, 255./ max);//颜色做了归一化，这里不利于调试
    ...
    //把U_map的框转换到正式世界的坐标系的值，单位是cm(这里表示存疑)
