...
namespace onboardDetector{
    dynamicDetector::dynamicDetector(const rclcpp::NodeOptions& options) : Node("dynamic_detector_node", options){
    ...
    void dynamicDetector::initParam(){
        ...
        // UV detector threshold point
        this->declare_parameter<int>("uv_threshold_point", 3);
        this->get_parameter("uv_threshold_point", this->uvThresholdPoint_);
        std::cout << this->hint_ << ": UV detector threshold point is set to: " << this->uvThresholdPoint_ << std::endl;

        this->declare_parameter<double>("uv_min_height", 0.0);
        this->get_parameter("uv_min_height", this->uvMinHeight_);
        std::cout << this->hint_ << ": UV detector min height is set to: " << this->uvMinHeight_ << std::endl;

        this->declare_parameter<double>("uv_max_height", 100.0);
        this->get_parameter("uv_max_height", this->uvMaxHeight_);
        std::cout << this->hint_ << ": UV detector max height is set to: " << this->uvMaxHeight_ << std::endl;
    ...
    void dynamicDetector::registerPub(){
        ...
        // raw filtered depth cloud pub (before dbscan/voxel filtering)
        this->filteredPointCloudPub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(this->ns_ + "/raw_filtered_depth_cloud", 10);
        ...
        // yolo uv map nearest center pub
        this->yoloUvCenterPub_ = this->create_publisher<geometry_msgs::msg::PointStamped>(this->ns_ + "/yolo_uv_nearest_center", 10);
    }
    ...
    void dynamicDetector::registerCallback(){
		...
		this->depthSub_ = this->create_subscription<sensor_msgs::msg::CompressedImage>(this->depthTopicName_, qos_profile, std::bind(&dynamicDetector::depthImgCB, this, std::placeholders::_1));


        // color image subscriber
        this->colorImageSub_ = this->create_subscription<sensor_msgs::msg::CompressedImage>(this->colorImgTopicName_, qos_profile, std::bind(&dynamicDetector::colorImgCB, this, std::placeholders::_1));
        ...
        this->yoloKeypointSub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>("yolo_detector/detected_keypoints", 10, std::bind(&dynamicDetector::yoloKeypointCB, this, std::placeholders::_1));
    ...
    void dynamicDetector::depthImgCB(const sensor_msgs::msg::CompressedImage::ConstSharedPtr& img){
        // store current depth image
        cv_bridge::CvImagePtr imgPtr = std::make_shared<cv_bridge::CvImage>();
        try {
            if (img->format.find("compressedDepth") != std::string::npos) {
                // Remove 12-byte header used by image_transport compressedDepth
                std::vector<uint8_t> img_data(img->data.begin() + 12, img->data.end());
                imgPtr->image = cv::imdecode(img_data, cv::IMREAD_UNCHANGED);
                imgPtr->encoding = sensor_msgs::image_encodings::TYPE_16UC1;
            } else {
                imgPtr = cv_bridge::toCvCopy(img);
            }
            if (imgPtr->image.empty()) {
                RCLCPP_WARN(this->get_logger(), "Decoded depth image is EMPTY in depthImgCB!");
            }
        ...
        // Since we are no longer subscribing to Odom/Pose, we initialize the poses statically to Identity.
        // This acts as a proxy to keep the rest of the node functioning properly by acting in a relative sensor frame.
        this->position_ = Eigen::Vector3d(0, 0, 0);
        this->orientation_ = Eigen::Matrix3d::Identity();

        this->positionDepth_ = Eigen::Vector3d(0, 0, 0);
        this->orientationDepth_ = Eigen::Matrix3d::Identity();

        this->positionColor_ = Eigen::Vector3d(0, 0, 0);
        this->orientationColor_ = Eigen::Matrix3d::Identity();
    }

    void dynamicDetector::colorImgCB(const sensor_msgs::msg::CompressedImage::ConstSharedPtr& img){
    ...
    void dynamicDetector::yoloKeypointCB(const std_msgs::msg::Float32MultiArray::ConstSharedPtr& keypoints){
        if (keypoints->data.empty()) return;
        
        vision_msgs::msg::Detection2DArray bbox_array;
        bbox_array.header.stamp = this->get_clock()->now();
        
        std::vector<std::vector<Eigen::Vector2d>> parsed_kpts;
        
        int num_detections = keypoints->data[0];
        int index = 1;
        
        for (int i = 0; i < num_detections; ++i) {
            float class_id = keypoints->data[index++];
            float conf = keypoints->data[index++];
            float x1 = keypoints->data[index++];
            float y1 = keypoints->data[index++];
            float x2 = keypoints->data[index++];
            float y2 = keypoints->data[index++];
            
            vision_msgs::msg::Detection2D bbox_msg;
            bbox_msg.bbox.center.position.x = x1;
            bbox_msg.bbox.center.position.y = y1;
            bbox_msg.bbox.size_x = std::abs(x2 - x1);
            bbox_msg.bbox.size_y = std::abs(y2 - y1);
            
            bbox_array.detections.push_back(bbox_msg);
            
            int num_kpts = keypoints->data[index++];
            std::vector<Eigen::Vector2d> kpts;
            for (int k = 0; k < num_kpts; ++k) {
                float kpt_x = keypoints->data[index++];
                float kpt_y = keypoints->data[index++];
                kpts.push_back(Eigen::Vector2d(kpt_x, kpt_y));
            }
            parsed_kpts.push_back(kpts);
        }
        
        this->yoloDetectionResults_ = bbox_array;
        this->parsedKeypoints_ = parsed_kpts;
    }

    void dynamicDetector::detectionCB(){
        // update pose history
        // this->updatePoseHist(); //存储并更新机器人的历史位姿,一个队列。
        // this->dbscanDetect(); //进行dbscan聚类，获取障碍物的3D边界框。好奇怪，dbscan不像这么稳的，但是实际效果好像还挺稳的，可视化看一下
        
        auto start_uv = std::chrono::high_resolution_clock::now();
        this->uvDetect(); //进行uv检测，获取障碍物的3D边界框,u就是umap了，v其实就是深度图（v应该是深度图的），uv联合计算标定框。
        auto end_uv = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> uv_duration = end_uv - start_uv;

        auto start_filter = std::chrono::high_resolution_clock::now();
        this->filterBBoxes(); //过滤边界框。
        auto end_filter = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> filter_duration = end_filter - start_filter;
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(now - last_print_time).count() >= 1) {
            std::cout << "[Timing] uvDetect took: " << uv_duration.count() << " ms" << std::endl;
            std::cout << "[Timing] filterBBoxes took: " << filter_duration.count() << " ms" << std::endl;
            last_print_time = now;
        }
    ...
    void dynamicDetector::visCB(){
        ...
        // publish raw filtered depth pointcloud before clustering in camera frame
        std::vector<Eigen::Vector3d> projPointsVec;
        for (int i=0; i<this->projPointsNum_; ++i){
            Eigen::Vector3d pCam = this->projPointsCam_[i];
            // Convert OpenCV optical frame (Z forward, X right, Y down) 
            // to ROS standard frame (X forward, Y left, Z up)
            Eigen::Vector3d pRos(pCam(2), -pCam(0), -pCam(1));
            projPointsVec.push_back(pRos);
        }
        this->publishPoints(projPointsVec, this->filteredPointCloudPub_, "base_link");
        
        // publish nearest YOLO-UV detected 3D bounding box center relative to the camera origin
        if (this->uvDetector_ != nullptr && !this->uvDetector_->box3Ds.empty()) {
            Eigen::Vector3d nearest_pRos;
            double min_dist_sq = std::numeric_limits<double>::max();
            bool found = false;
            int nearest_yolo_idx = -1;
            double nearest_box_z = 0.0;

            for (const auto& box : this->uvDetector_->box3Ds) {
                // UVDetector stores points in OpenCV frame (Z forward, X right, Y down).
                // Convert to ROS standard frame (X forward, Y left, Z up).
                Eigen::Vector3d pRos(box.z, -box.x, -box.y);
                
                double dist_sq = pRos.squaredNorm();
                if (dist_sq < min_dist_sq) {
                    min_dist_sq = dist_sq;
                    nearest_pRos = pRos;
                    nearest_yolo_idx = box.yolo_idx;
                    nearest_box_z = box.z;
                    found = true;
                }
            }
            ...
            if (found) {
                ...
                center_msg.header.frame_id = "base_link";
                
                // If keypoints exist for this detection (at least 3 keypoints), use the 3rd one
                if (nearest_yolo_idx != -1 && 
                    nearest_yolo_idx < (int)this->parsedKeypoints_.size() && 
                    this->parsedKeypoints_[nearest_yolo_idx].size() >= 3) {
                    
                    // 3rd keypoint has index 2
                    float kpt_x = this->parsedKeypoints_[nearest_yolo_idx][2].x();
                    float kpt_y = this->parsedKeypoints_[nearest_yolo_idx][2].y();
                    
                    // Inverse projection to 3D OpenCV camera frame
                    // nearest_box_z is already in meters.
                    float Z_cam = nearest_box_z;
                    float X_cam = (kpt_x - this->uvDetector_->px) * Z_cam / this->uvDetector_->fx;
                    float Y_cam = (kpt_y - this->uvDetector_->py) * Z_cam / this->uvDetector_->fy;
                    
                    // Convert OpenCV standard frame to ROS standard frame (X forward, Y left, Z up)
                    center_msg.point.x = Z_cam;
                    center_msg.point.y = -X_cam;
                    center_msg.point.z = -Y_cam;
                    
                    // RCLCPP_INFO(this->get_logger(), "[dynamicDetector] Publishing 3rd keypoint 3D coordinate: (%.2f, %.2f, %.2f)", center_msg.point.x, center_msg.point.y, center_msg.point.z);
                } else {
                    // Fallback to 3D bounding box center
                    center_msg.point.x = nearest_pRos.x();
                    center_msg.point.y = nearest_pRos.y();
                    center_msg.point.z = nearest_pRos.z();
                }
                
                this->yoloUvCenterPub_->publish(center_msg);
            }
        }
    ...
    void dynamicDetector::dbscanDetect(){
        ...
        // 2. filter points
        this->filterPoints(this->projPoints_, this->filteredPoints_);//全局系下的
    ...
    void dynamicDetector::uvDetect(){
        ...
        if (this->uvDetector_ == NULL){
            ...
            this->uvDetector_->threshold_point = this->uvThresholdPoint_;
            this->uvDetector_->min_height = this->uvMinHeight_;
            this->uvDetector_->max_height = this->uvMaxHeight_;
        }

        // Extract ROIs from YOLO detection results
        this->uvDetector_->rois_.clear();
        for (const auto& detection : this->yoloDetectionResults_.detections) {
            ...
            // Expand by 20%
            float margin_x = w * 0.1f;
            float margin_y = h * 0.1f;

            int roi_x = std::max(0, static_cast<int>(x - margin_x));
            int roi_y = std::max(0, static_cast<int>(y - margin_y));
            int roi_w = static_cast<int>(w + 2 * margin_x);
            int roi_h = static_cast<int>(h + 2 * margin_y);

            if (!this->depthImage_.empty()) {
                roi_w = std::min(roi_w, this->depthImage_.cols - roi_x);
                roi_h = std::min(roi_h, this->depthImage_.rows - roi_y);
            }

            this->uvDetector_->rois_.push_back(cv::Rect(roi_x, roi_y, roi_w, roi_h));
        }
        ...
        if (not this->depthImage_.empty()){
            ...
            // Generate 3D boxes from UV bounding boxes using YOLO's y-coordinates
            this->uvDetector_->box3Ds.clear();
            this->uvDetector_->bounding_box_D.clear();

            float histSize = this->uvDetector_->depth.rows / this->uvDetector_->row_downsample;
            float bin_width = ceil((this->uvDetector_->max_dist - this->uvDetector_->min_dist) / histSize);
            float iou_threshold = 0.3f; // configurable IoU threshold param，iou

            for (size_t b = 0; b < this->uvDetector_->bounding_box_U.size(); b++) {
                float uv_x = this->uvDetector_->bounding_box_U[b].tl().x;
                float uv_width = this->uvDetector_->bounding_box_U[b].width;
                int bin_index_small = this->uvDetector_->bounding_box_U[b].tl().y;
                int bin_index_large = this->uvDetector_->bounding_box_U[b].br().y;
                
                float depth_in_near = (bin_index_small * bin_width + this->uvDetector_->min_dist);
                float depth_of_depth = (bin_index_large - bin_index_small) * bin_width;
                float depth_in_far = depth_of_depth * 1.3f + depth_in_near;

                float im_frame_x = (uv_x + uv_width / 2.0f) / this->uvDetector_->col_scale;
                float im_frame_x_width = uv_width / this->uvDetector_->col_scale;
                
                float uv_x_min = uv_x / this->uvDetector_->col_scale;
                float uv_x_max = uv_x_min + im_frame_x_width;

                float best_iou = 0.0f;
                int best_yolo_idx = -1;

                for (size_t i = 0; i < this->yoloDetectionResults_.detections.size(); i++) {
                    const auto& detection = this->yoloDetectionResults_.detections[i];
                    float yolo_x_min = detection.bbox.center.position.x;
                    float yolo_y_min = detection.bbox.center.position.y;
                    float yolo_w = detection.bbox.size_x;
                    float yolo_h = detection.bbox.size_y;
                    float yolo_x_max = yolo_x_min + yolo_w;

                    // Assume YOLO's y for UV map box to compute intersection
                    float inter_x_min = std::max(uv_x_min, yolo_x_min);
                    float inter_x_max = std::min(uv_x_max, yolo_x_max);
                    float inter_w = std::max(0.0f, inter_x_max - inter_x_min);

                    float inter_area = inter_w * yolo_h;
                    float uv_box_area = im_frame_x_width * yolo_h;
                    float yolo_box_area = yolo_w * yolo_h;
                    float union_area = uv_box_area + yolo_box_area - inter_area;
                ...
                if (best_iou >= iou_threshold && best_yolo_idx != -1) {
                    const auto& detection = this->yoloDetectionResults_.detections[best_yolo_idx];
                    float y_up = detection.bbox.center.position.y;
                    float y_down = y_up + detection.bbox.size_y;

                    // Prevent bounding box leaving the image height dimensions
                    if (y_up < 0) y_up = 0;
                    if (y_down > this->depthImage_.rows) y_down = this->depthImage_.rows;

                    // Compute 3D Bounding Box
                    float im_frame_y = (y_down + y_up) / 2.0f;
                    float im_frame_y_width = y_down - y_up;
                    float Y_w = (depth_in_near + depth_in_far) / 2.0f;

                    onboardDetector::box3D curr_box;
                    curr_box.x = (im_frame_x - this->uvDetector_->px) * Y_w / this->uvDetector_->fx;
                    curr_box.y = (im_frame_y - this->uvDetector_->py) * Y_w / this->uvDetector_->fy;
                    curr_box.x_width = im_frame_x_width * Y_w / this->uvDetector_->fx;
                    curr_box.y_width = im_frame_y_width * Y_w / this->uvDetector_->fy;
                    curr_box.z = Y_w;
                    ...
                    curr_box.y /= 1000.0f;
                    curr_box.z /= 1000.0f;
                    curr_box.x_width /= 1000.0f;
                    curr_box.y_width /= 1000.0f;
                    curr_box.z_width /= 1000.0f;
                    
                    curr_box.yolo_idx = best_yolo_idx;

                    this->uvDetector_->box3Ds.push_back(curr_box);
                    this->uvDetector_->bounding_box_D.push_back(cv::Rect((int)uv_x_min, (int)y_up, (int)im_frame_x_width, (int)im_frame_y_width));
                }
            }
    ...
    // filterBBoxes()的作用就是**“相亲”与“融合”：它把两路算法找出来的框拿来对比验证。
    // 如果这两路算法在空间上的同一个位置都发现了同一个障碍物，那说明这个检测非常可靠**，
    // 将其融合成一个更为准确的 Bounding Box，进入下一步。如果不匹配，就作为不可靠的检测被抛弃。
    // 下面我们逐行看它的双向匹配与融合逻辑
    void dynamicDetector::filterBBoxes(){
        ...
        // 遍历 UV 算法得出的每一个框，直接将其作为有效框（抛弃 DB 检测的融合验证）
        for (size_t i=0 ; i<this->uvBBoxes_.size(); ++i){
            onboardDetector::box3D uvBBox = this->uvBBoxes_[i];

            filteredBBoxesTemp.push_back(uvBBox);
            
            // 为了不让后续基于点云特征的卡尔曼跟踪崩溃，填充虚拟的 PointCloud 数据
            std::vector<Eigen::Vector3d> dummyPcCluster;
            dummyPcCluster.push_back(Eigen::Vector3d(uvBBox.x, uvBBox.y, uvBBox.z));
            filteredPcClustersTemp.push_back(dummyPcCluster);      
            filteredPcClusterCentersTemp.push_back(Eigen::Vector3d(uvBBox.x, uvBBox.y, uvBBox.z));
            filteredPcClusterStdsTemp.push_back(Eigen::Vector3d(0.0, 0.0, 0.0));
        }
    ...
    void dynamicDetector::projectDepthImage(){
        ...
        for (int v=this->depthFilterMargin_; v<rows-this->depthFilterMargin_; v=v+this->skipPixel_){ // row
            ...
            for (int u=this->depthFilterMargin_; u<cols-this->depthFilterMargin_; u=u+this->skipPixel_){ // column
                ...
                // filter points based on height (camera y axis)
                if (currPointCam(1) < this->uvMinHeight_ || currPointCam(1) > this->uvMaxHeight_) {
                    continue;
                }

                currPointMap = this->orientationDepth_ * currPointCam + this->positionDepth_; // transform to map coordinate

                this->projPointsCam_[this->projPointsNum_] = currPointCam;
    ...
    void dynamicDetector::clusterPointsAndBBoxes(
        ...
        this->eigenToDBPointVec(points, pointsDB, points.size());//xyz+clusterID

        this->dbCluster_.reset(new DBSCAN (this->dbMinPointsCluster_, this->dbEpsilon_, pointsDB));

        // DBSCAN clustering，感觉就像病毒蔓延的感觉，一个点感染周围的点，然后被感染的点又感染周围的点，直到感染不动了
        this->dbCluster_->run();
        ...
        for (size_t i=0 ; i<pcClusters.size() ; ++i){
            ...
            this->calcPcFeat(pcClusters[i], pcClusterCenter, pcClusterStd);//求平均值，方差
    ...
    void dynamicDetector::calcPcFeat(
        const std::vector<Eigen::Vector3d>& pcCluster, 
    ...
    void dynamicDetector::publishUVImages(){
        ...
        if (!this->uvDetector_->depth_show.empty()) {
            sensor_msgs::msg::Image::SharedPtr depthBoxMsg = cv_bridge::CvImage(header, "bgr8", this->uvDetector_->depth_show).toImageMsg();
            this->uvDepthMapPub_->publish(*depthBoxMsg);
        }
        if (!this->uvDetector_->U_map_show.empty()) {
            sensor_msgs::msg::Image::SharedPtr umapBoxMsg = cv_bridge::CvImage(header, "bgr8", this->uvDetector_->U_map_show).toImageMsg();
            this->uDepthMapPub_->publish(*umapBoxMsg); 
        }
        if (!this->uvDetector_->bird_view.empty()) {
            sensor_msgs::msg::Image::SharedPtr birdBoxMsg = cv_bridge::CvImage(header, "bgr8", this->uvDetector_->bird_view).toImageMsg();  
    ...
    void dynamicDetector::publishPoints(const std::vector<Eigen::Vector3d>& points, const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr& publisher, const std::string& frame_id){
